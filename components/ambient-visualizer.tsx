...
export function AmbientVisualizer({ type, isHovered, theme }: AmbientVisualizerProps) {
  ...
  const stateRef = useRef<{
    initialized: boolean;
    // Headphones state
    ripples: { r: number; alpha: number; maxR: number; speed: number }[];
    // Carafe (water) state
    bubbles: { x: number; y: number; size: number; speedY: number; swing: number; phase: number }[];
    // Incense (smoke/gas) state
    smokeParticles: { x: number; y: number; vx: number; vy: number; size: number; alpha: number; life: number; maxLife: number }[];
    // Keyboard state
    keyGrid: { cx: number; cy: number; activeIntensity: number; phase: number }[];
  }>({
    initialized: false,
    ripples: [],
    bubbles: [],
  ...
  useEffect(() => {
    ...
    // Initialize states once or on container dimension changes
    const w = canvas.offsetWidth;
    const h = canvas.offsetHeight;

    if (!state.initialized && w > 0 && h > 0) {
      // 1. Water Carafe (Bubbles)
    ...
    const draw = () => {
      ...
      // =======================================================================
      // THEMED SPECIFIC VISUALIZERS (Extremely High-End, Subtle & Elegant)
      // =======================================================================

      if (type === "headphones") {
        // =======================================================================
        // 1. AUDIO / MUSIC SOUNDWAVES & PULSES (Headphones)
        // =======================================================================
        // Draw elegant concentric ripple rings expanding outwards from the center
        // and organic horizontal sound waves

        const centerX = w * 0.45;
        const centerY = h * 0.45;

        // Pulse audio trigger rings on hover or slowly on idle
        ...
        // Elegant flowing soundwave path across the bottom card
        ctx.beginPath();
        ctx.lineWidth = 0.5;
        ...
        // =======================================================================
        // 2. MECHANICAL KEYPRESS GRID MATRIX (Keyboard)
        // =======================================================================
        // Draw tiny matrix dots that light up and form elegant glowing grid points
        // reacting to the mouse location
        ...
        state.keyGrid.forEach((point) => {
          ...
          // Subtle horizontal link lines in active region
        ...
        // =======================================================================
        // 3. FLUID LIQUID WAVES & RISING BUBBLES (Water Carafe)
        // =======================================================================
        
        // Fluid waves rippling at the base of the card
        const waveHeight = isHovered ? 8 : 4;
        const waveY = h * 0.85;

        // Wave 1 (Back, darker/softer)
        ...
        // =======================================================================
        // 4. ORGANIC RISING SMOKE / GAS DRIFT (Incense Burner)
        // =======================================================================
        
        // Spawn smoke particles slowly
        ...
        // Draw and update smoke particles with a organic drifting wisp effect
        state.smokeParticles.forEach((p, idx) => {
          ...
          // Gentle breeze drift + hover cursor wind interaction
          const waveDrift = Math.sin(p.life * 0.02 + idx) * 0.08;
          p.vx += waveDrift;

          if (isHovered) {
            // Smoke gets gently drawn or pushed by the mouse
          ...
          // Gradually expand size and fade out opacity
