...
export default function CategoryPage({
  ...
  // Wrapped in React.startTransition to activate the @vercel-react-view-transitions framework
  ...
  // Filter products: If category is "all", fetch all products; else filter by category id
  const categoryProducts =
    id === "all" ? PRODUCTS : PRODUCTS.filter((p) => p.category === id);

  // Apply price dynamic filtering
  ...
  const sortedProducts = [...filteredProducts].sort((a, b) => {
    ...
    return 0; // Default Featured (order in database)
  });
  ...
  return (
    <div className="relative w-full bg-background text-foreground overflow-x-hidden font-sans select-none transition-colors duration-500 flex flex-col">
      ...
      <div className="relative w-full h-[520px] shrink-0 overflow-hidden bg-card z-20 flex items-center justify-center">
        {/* Micro-Profile Blending Gradient */}
        ...
        {/* Bespoke Full-Width Curved Cutout */}
        <svg
          viewBox="0 0 1000 100"
          preserveAspectRatio="none"
          className="absolute bottom-0 left-0 w-full h-10 text-background fill-current z-25 pointer-events-none transition-colors duration-500"
        >
          <path d="M0,100 L1000,100 L1000,0 C750,80 250,80 0,0 Z" />
        </svg>

        {/* Elegant global dark wash to ensure perfect readability of the white typography */}
        <div className="absolute inset-0 bg-black/20 dark:bg-black/35 z-10 pointer-events-none" />

        {/* Solid Lifestyle Background Photo */}
        <Image
          src={meta.image}
          alt={meta.title}
          fill
        ...
        {/* Clean, Unboxed Typography Panel with refined drop shadows */}
      ...
      {/* Mobile: explicit cue — filters are directly under the hero (not at page bottom) */}
      <p
        className="z-30 mx-auto w-full max-w-[1600px] px-4 pt-5 pb-0 text-center font-mono text-[9px] uppercase tracking-[0.22em] text-muted-foreground lg:hidden"
        role="note"
      >
      ...
      {/* 
        ================= MAIN DIRECTORY GRID & SIDEBAR FILTERS =================
        Locked to a maximum 1600px width with elegant px-16 padding.
        Laid out as a 5-column grid: 1 column for the sticky sidebar filter, 
        and 4 columns for the breathtaking product catalog grid.
      */}
      <div className="relative w-full max-w-[1600px] mx-auto px-4 sm:px-8 lg:px-16 py-10 lg:py-24 z-30 flex-1 grid grid-cols-1 lg:grid-cols-5 gap-8 lg:gap-16">
        {/* Filters first on all breakpoints — sidebar on lg */}
        <div className="col-span-1 flex flex-col gap-6 lg:gap-12 lg:sticky lg:top-32 h-fit select-none">
          ...
          {/* Price + sort: side-by-side from sm → saves vertical space on phones in landscape / small tablets; stacked on narrow mobile */}
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-1 gap-8 lg:gap-12">
          {/* Filtering Group 1: Price Spectrum */}
          <div className="flex flex-col gap-2 sm:gap-3">
            ...
            <button
              type="button"
              onClick={() => handlePriceFilterChange("under-150")}
              className={`font-mono text-[10.5px] tracking-widest uppercase text-left transition-all duration-300 py-1 cursor-pointer flex items-center gap-2 focus:outline-none ${
            ...
            <button
              type="button"
              onClick={() => handlePriceFilterChange("under-300")}
              className={`font-mono text-[10.5px] tracking-widest uppercase text-left transition-all duration-300 py-1 cursor-pointer flex items-center gap-2 focus:outline-none ${
            ...
            <button
              type="button"
              onClick={() => handlePriceFilterChange("over-300")}
              className={`font-mono text-[10.5px] tracking-widest uppercase text-left transition-all duration-300 py-1 cursor-pointer flex items-center gap-2 focus:outline-none ${
          ...
          {/* Filtering Group 2: Chronological Sort */}
          <div className="flex flex-col gap-2 sm:gap-3">
            <span className="font-mono text-[9px] tracking-[0.25em] text-muted-foreground/80 uppercase mb-1">
              CHRONOLOGY SORT
            </span>
        ...
        {/* Product grid — to the right on lg, below filters on mobile */}
        <div className="col-span-1 lg:col-span-4 flex flex-col gap-8 lg:gap-10 min-h-0">
          {sortedProducts.length === 0 ? (
            <div className="w-full py-20 flex flex-col items-center justify-center border border-dashed border-border/40">
